class LiveSurface {
  ...
  void update() {
    ...
    if (opencvLeft.findContours().size() > 0) {
      ...
      Contour contour = opencvLeft.findContours(false, true).get(0);  //Contours will be sorted by area, largest first
      //println(contour);


      //test for size of contour
      ...
      if (rectArea > 25) { //25  is the area of the chainshape. If more than 25, carry out the code below
        ...
        println("This is inside if statement"+rectArea);
